...
const SectionHeader = ({ title }: { title: string }) => (
  <h3 className="bg-[#008a00] text-white px-3 py-2 text-[11px] font-black uppercase tracking-widest mt-10 mb-4 flex items-center justify-between">
    <span>{title}</span>
    <span className="text-[8px] opacity-70">CONFIDENTIAL • TD INSTITUTIONAL BANKING</span>
  </h3>
);
    ...
    <div className="relative">
      <div className="sticky top-4 right-4 z-50 flex justify-end mb-6 gap-2 no-print">
        <button 
          ...
          className="flex items-center gap-2 px-5 py-2.5 bg-[#008a00] text-white rounded-lg text-xs font-black shadow-2xl hover:bg-[#007000] transition-all border border-green-700"
        >
      ...
      <div id="memo-document-root" className="bg-white p-12 lg:p-20 shadow-2xl border border-gray-300 min-h-[11in] w-full max-w-5xl mx-auto font-serif text-gray-900 leading-snug print:shadow-none print:border-none">
        ...
        {/* TOP BRANDING */}
        <div className="flex justify-between items-start border-b-4 border-[#008a00] pb-4 mb-8">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tighter">Corporate Credit Review</h1>
            <p className="text-xs font-bold text-[#008a00] tracking-widest mt-1 italic underline decoration-2 underline-offset-4">SYNDICATED LOAN UNDERWRITING UNIT</p>
          </div>
          <div className="text-right text-[9px] uppercase font-bold text-slate-600">
            <p>Form ID: {memo?.header?.reviewId || 'PENDING'}</p>
          ...
          <div className="mt-4 grid grid-cols-3 gap-y-2 bg-slate-50 p-3 border text-[9px] font-bold uppercase">
            {Object.entries(memo.borrowerDetails.flags).map(([key, val]) => (
              <div key={key} className="flex items-center gap-2">
                <span className={`w-3 h-3 border ${val ? 'bg-[#008a00]' : 'bg-white'}`}></span>
                {key.replace(/([A-Z])/g, ' $1')}
              </div>
            ))}
          </div>
        ...
        <table className="w-full text-[9px] border-collapse border border-slate-300">
          ...
          <tbody>
            {safeArray(memo?.creditPosition).map((cp, idx) => (
        ...
        <div className="grid grid-cols-2 gap-x-10">
          ...
          <TableRow label="Economic RAROC (%)" value={memo?.raroc?.projectedEconomicRaroc !== undefined ? `${memo.raroc.projectedEconomicRaroc}%` : '---'} />
          <TableRow label="Relationship RAROC (%)" value={memo?.raroc?.projectedRelationshipRaroc !== undefined ? `${memo.raroc.projectedRelationshipRaroc}%` : '---'} />
        ...
        <table className="w-full text-[9px] border-collapse border border-slate-300">
           ...
           <tbody>
             {safeArray(memo?.covenants?.financial).map((cf, idx) => (
               <tr key={idx} className="text-center">
                 ...
                 <td className="border border-slate-300 p-1 font-bold text-[#008a00]">{cf.inCompliance ? 'YES' : 'NO'}</td>
               </tr>
             ))}
           </tbody>
        </table>
        ...
        <div className="mt-12 grid grid-cols-3 gap-10">
          <div className="border-t-2 border-slate-900 pt-2 text-[8px] uppercase font-black">
        ...
        <div className="mt-32 border-t-2 border-slate-200 pt-6 text-center">
          <p className="text-[8px] tracking-[0.4em] uppercase font-black text-slate-400">Strictly Confidential - For Internal TD Syndicated Underwriting Only</p>
        </div>
      </div>
    </div>
  );
